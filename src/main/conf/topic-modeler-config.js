...
module.exports = function (db, lda) {
  ...
  $TM.getTopics = function (sentences) {
    var documents = new Array(),
        ...
        // List of words to be skipped by the topic modelling
    ...
    for (var i = 0; i < sentences.length; i++) {
      ...
      for(var wc=0;wc<words.length;wc++) {
        ...
        if (w=="" || w.length==1 || w.indexOf("http")==0 || stopwords[w] === 1) continue;
        if (f[w]) { 
          f[w]=f[w]+1;      
        } 
        else if(w) { 
  ...
  // Finds the topics associated with the query domain
  $TM.findDomainTopics = function (queryDomain) {
    var currentArchdomain,
        currentDomain;
    for (var archdomain in $TM.topicTree) {
      currentArchdomain = $TM.topicTree[archdomain];
      for (var domain in currentArchdomain) {
        var currentDomain = currentArchdomain[domain];
